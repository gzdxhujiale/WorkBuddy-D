...
function localRows(): FocusSession[] {
  try { return JSON.parse(localStorage.getItem(LOCAL_KEY) ?? "[]") as FocusSession[]; } catch { return []; }
}
function saveLocal(rows: FocusSession[]) { localStorage.setItem(LOCAL_KEY, JSON.stringify(rows)); }
...
export const focusAssistantApi = {
  async create(input: CreateSession): Promise<FocusSession> {
    ...
    if (error) console.warn("Focus session cloud save failed:", error.message);
    return session;
  },
  async update(id: string, updates: Partial<Pick<FocusSession, "status" | "activeSeconds" | "restCompleted" | "endedAt">>): Promise<void> {
    ...
    if (error) console.warn("Focus session cloud update failed:", error.message);
  },
  async markOpenSessionsInterrupted(): Promise<void> {
    ...
    if (error) console.warn("Focus session interruption save failed:", error.message);
  },
  fromDb,
};
