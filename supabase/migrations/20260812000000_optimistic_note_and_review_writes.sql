-- Atomic optimistic writes for rich-text notes and daily reviews. JavaScript
-- dates have millisecond precision, so comparisons are normalized accordingly.
...
begin
  if p_expected_updated_at is null then
    ...
    update public.habits set name = p_name, frequency_type = p_frequency_type, frequency_days = p_frequency_days,
      goal = p_goal, start_date = p_start_date, duration = p_duration, category = p_category,
      reminder = p_reminder, auto_popup_log = p_auto_popup_log, sort_order = p_sort_order, updated_at = p_next_updated_at
...
create or replace function public.save_note(
  ...
  p_is_pinned boolean, p_sort_order integer, p_created_at timestamptz,
  p_expected_updated_at timestamptz, p_next_updated_at timestamptz
) returns timestamptz language plpgsql security invoker set search_path = public as $$
declare saved_at timestamptz;
begin
  if p_expected_updated_at is null then
    ...
    update public.notes set folder_id = p_folder_id, group_id = p_group_id, title = p_title,
      content = p_content, is_pinned = p_is_pinned, sort_order = p_sort_order, updated_at = p_next_updated_at
    where id = p_id and user_id = auth.uid() and deleted_at is null
      and date_trunc('milliseconds', updated_at) = date_trunc('milliseconds', p_expected_updated_at)
    returning updated_at into saved_at;
  end if;
