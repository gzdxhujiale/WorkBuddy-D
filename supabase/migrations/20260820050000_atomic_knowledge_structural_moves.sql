-- A structural drag changes membership and an entire sibling order as one
-- versioned operation.  Partial client-side reorder persistence is forbidden.

create function public.move_and_reorder_notes_v3(
  p_id uuid,
  p_folder_id uuid,
  p_group_id uuid,
  p_title text,
  p_content text,
  p_content_loaded boolean,
...
declare
  ...
  item_count integer;
  distinct_sort_count integer;
  min_sort_order integer;
...
begin
  ...
  if p_group_id is not null and not exists (
    ...
    where group_row.id = p_group_id
      and group_row.folder_id = p_folder_id
  ...
  if p_items is null then
    ...
    where note.user_id = (select auth.uid())
      ...
      and note.folder_id = p_folder_id
      and note.group_id is not distinct from p_group_id
      and note.id <> p_id;
    ...
    set folder_id = p_folder_id,
        group_id = p_group_id,
        title = p_title,
        content = case when p_content_loaded then p_content else note.content end,
  ...
  if item_count = 0 or distinct_sort_count <> item_count
     or min_sort_order <> 0 or max_sort_order <> item_count - 1 then
    raise exception 'INVALID_NOTE_ORDER' using errcode = '22023';
  end if;
  ...
  if exists (
    ...
    raise exception 'DUPLICATE_NOTE_ORDER_ITEM' using errcode = '22023';
  end if;
  ...
  if exists (
    ...
    where item.lock_version is null or item.sort_order is null
      or note.id is null or note.user_id <> (select auth.uid())
      or note.deleted_at is not null or note.lock_version <> item.lock_version
  ) then
  ...
  if exists (
    ...
    select 1
    where (select count(*) from supplied) <> (select count(*) from target)
  ...
  return query
  update public.notes note
  set folder_id = case when note.id = p_id then p_folder_id else note.folder_id end,
  ...
  where note.id = item.id
    and note.lock_version = item.lock_version
  returning note.id, note.updated_at, note.lock_version, note.sort_order;
  get diagnostics updated_item_count = row_count;
  if updated_item_count <> item_count then
    raise exception 'VERSION_CONFLICT' using errcode = '40001';
  end if;
end;
$$;

create function public.move_and_reorder_knowledge_base_folders_v3(
  p_id uuid,
...
begin
  if p_knowledge_base_id is not null and not exists (
    ...
    where base.id = p_knowledge_base_id
      and base.user_id = (select auth.uid())
      and base.deleted_at is null
  ) then
    raise exception 'KNOWLEDGE_BASE_NOT_FOUND' using errcode = 'P0002';
  end if;

  if p_items is null then
    perform pg_advisory_xact_lock(
      hashtextextended(coalesce(p_knowledge_base_id::text, 'root'), 0)
    );
    ...
    where folder.user_id = (select auth.uid())
      and folder.deleted_at is null
      and folder.knowledge_base_id is not distinct from p_knowledge_base_id
      and folder.id <> p_id;

    return query
    update public.knowledge_base_folders folder
    set knowledge_base_id = p_knowledge_base_id,
        name = p_name,
        sort_order = assigned_sort_order
    where folder.id = p_id
      and folder.user_id = (select auth.uid())
      and folder.deleted_at is null
  ...
  if jsonb_typeof(p_items) <> 'array' then
    raise exception 'INVALID_FOLDER_ORDER' using errcode = '22023';
  end if;
  ...
  if exists (
    ...
    raise exception 'DUPLICATE_FOLDER_ORDER_ITEM' using errcode = '22023';
  end if;
  ...
  if exists (
    ...
    where item.lock_version is null or item.sort_order is null
      or folder.id is null or folder.user_id <> (select auth.uid())
      or folder.deleted_at is not null or folder.lock_version <> item.lock_version
  ) then
    raise exception 'VERSION_CONFLICT' using errcode = '40001';
  end if;
  ...
  return query
  update public.knowledge_base_folders folder
  set knowledge_base_id = case when folder.id = p_id then p_knowledge_base_id else folder.knowledge_base_id end,
      name = case when folder.id = p_id then p_name else folder.name end,
      sort_order = item.sort_order
  from jsonb_to_recordset(p_items) as item(id uuid, sort_order integer, lock_version bigint)
  where folder.id = item.id
    and folder.lock_version = item.lock_version
  returning folder.id, folder.updated_at, folder.lock_version, folder.sort_order;
  get diagnostics updated_item_count = row_count;
  if updated_item_count <> item_count then
    raise exception 'VERSION_CONFLICT' using errcode = '40001';
