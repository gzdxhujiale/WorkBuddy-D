...
| 可编辑实体的乐观锁版本 `lock_version` | 数据库 | 单调递增触发器；客户端传 `p_expected_lock_version`，成功后采用 RPC 返回的新值 |
...
抽屉关闭并不会卸载编辑器（它会先执行离场动画），因此关闭时必须先提交当前标题/正文草稿，再立即 drain 该笔记的防抖写入；不能只依赖组件卸载或 3 秒计时器。
...
| 离线后保存支持队列的实体并恢复网络 | 每个实体只保留最新待重放操作；不可恢复或找不到执行器的操作显式保留为冲突，损坏的队列原文会被保留到恢复键而非静默丢弃。 |
