...
| Frontend engineering | 3 | Strict TypeScript, lazy routes, Query/Zustand split, domain services/hooks. | No frontend test/lint/format workflow; one synchronization design divergence exists. |
...
| Documentation legibility | 4 | System docs, design decisions, product index, migration history, generated live snapshot, and skills are present. | Some legacy schema terminology and code/design synchronization divergence need reconciliation. |
...
3. Reconcile the list-note Tauri cache-patch implementation with the accepted Broadcast-as-invalidation design decision.
